...
- **`apa_genome/{GENE}[.group].png`** — structure only, no usage/expression. The whole gene at real scale on top, and a `ggforce::facet_zoom()` inset of just the terminal-exon/PAS region below, connected by an automatic shaded funnel — tightly clustered PAS (e.g. 5 within ~1kb) are illegible at whole-gene scale, so the inset exists purely to make that count legible. Every annotated RefSeq transcript is its own row, with each exon colored **Constitutive** (grey — identical start/end shared by every other isoform that overlaps this genomic region) or **Alternative** (orange — this isoform splices differently here, or this is an alternative/variant terminal exon). Answers "how many isoforms are there, and where do they structurally differ (alternative splicing, alternative terminal exons)?" at a glance. Isoforms at a completely different, unrelated locus (e.g. a retained-intron transcript far upstream) are still shown as a row in both panels — in the zoomed one it's simply empty, since that isoform has no data there, which is accurate rather than a filtering choice — and are correctly excluded from the Constitutive/Alternative comparison for exons they don't overlap: a transcript starting from a different promoter isn't "alternative splicing" everywhere the two don't overlap, it's just a different locus. If a gene's isoforms have no structural differences at all, the caption says so explicitly.
